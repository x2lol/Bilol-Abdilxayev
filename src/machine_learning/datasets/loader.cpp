...
#include <machine_learning/datasets/loader.hpp>
...
IDXDataset MNISTLoader::load(const std::string& base_path) {
    return IDXLoader::load(fmt::format("{}/train-images-idx3-ubyte", base_path), fmt::format("{}/train-labels-idx1-ubyte", base_path));
}


IDXDataset EMNISTLoader::load(const std::string& base_path, const std::string& split) {
    const std::string images = fmt::format("{}/emnist-{}-train-images-idx3-ubyte", base_path, split);
    const std::string labels = fmt::format("{}/emnist-{}-train-labels-idx1-ubyte", base_path, split);
