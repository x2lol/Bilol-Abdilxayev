...
namespace network::serialization {
    ...
    void ModelIO::load(const std::string& path, std::vector<float>& weights, std::vector<float>& biases) {
        ...
        if(!file.read(reinterpret_cast<char*>(&w), sizeof(w))) throw loader::FileReadException(fmt::format("at path: {} while trying to infer size of 'weights'", path));
        weights.resize(w);
        if(!file.read(reinterpret_cast<char*>(weights.data()), w * sizeof(float))) throw loader::FileReadException(fmt::format("at path: {} while trying to read 'weights'", path));

        if(!file.read(reinterpret_cast<char*>(&b), sizeof(b))) throw loader::FileReadException(fmt::format("at path: {} while trying to infer size of 'biases'", path));
        biases.resize(b);
        if(!file.read(reinterpret_cast<char*>(biases.data()), b * sizeof(float))) throw loader::FileReadException(fmt::format("at path: {} while trying to read 'biases'", path));
    }
