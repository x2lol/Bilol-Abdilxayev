...
public:
    ...
    T& operator()(size_t r, size_t c) {
        if (r >= rows_ || c >= cols_) {
            throw OutOfBounds(fmt::format("cannot access entry at ({}, {}) on a {}x{} matrix", r, c, rows_, cols_));
        }
    ...
    T& operator[](size_t i) {
        if (i >= this->size()) {
            throw OutOfBounds(fmt::format("cannot access elementy at {} in a matrix of size {}", i, this->size()));
        }
    ...
    Matrix operator+(const Matrix& other) const {
        ...
        if (rows_ != other.rows() || cols_ != other.cols()) {
            throw DimensionMismatch(fmt::format("cannot add {}x{} matrix to {}x{} matrix", other.rows(), other.cols(), rows_, cols_));
        }
    ...
    Matrix operator-(const Matrix& other) const {
        ...
        if (rows_ != other.rows() || cols_ != other.cols()) {
            throw DimensionMismatch(fmt::format("cannot substract {}x{} matrix from {}x{} matrix", other.rows(), other.cols(), rows_, cols_));
        }
    ...
    Matrix operator*(const Matrix<T>& other) const {
        ...
        if (cols_ != other.rows()) {
            throw DimensionMismatch(fmt::format("cannot multiply {}x{} matrix by {}x{} matrix", rows_, cols_, other.rows(), other.cols()));
        }
    ...
    Matrix hadamard(const Matrix<T>& other) const {
        if (rows_ != other.rows() || cols_ != other.cols()) {
            throw DimensionMismatch(fmt::format("cannot elementwise multiply {}x{} matrix by {}x{} matrix", other.rows(), other.cols(), rows_, cols_));
        }
        Matrix<T> res(rows_, cols_);
    ...
    Matrix add_colwise(const Matrix& other) const {
        ...
        if (other.cols() != 1 || other.rows() != rows_) {
            throw DimensionMismatch(fmt::format("cannot columnwise broadcast {}x{} matrix to {}x{} matrix", other.rows(), other.cols(), rows_, cols_));
        }
    ...
    T dot(const Matrix<T>& other) const {
        ...
        if (!this->is_vector() || !other.is_vector() || this->size() != other.size()) {
            throw DimensionMismatch( fmt::format("dot product is not defined for {}x{} and {}x{}", rows_, cols_, other.rows(), other.cols()));
        }
